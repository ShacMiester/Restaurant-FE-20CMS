...
export class CateringDetailsComponent extends CrudService<any, number> implements OnInit, OnDestroy {
  cateringForm$: any
    ...
    this.CateringService.getCateringForm().subscribe(
      {
        next: (v) => {
          this.cateringForm$ = v
          this.subscriptions.add(this.CateringService.applyNewField().subscribe({ next: (b) => this.cateringForm$ = b }))
        },
        complete: () => {
  ...
  cater(event: any) {
    this.subscriptions.add(this.update(event.payload, this.data.item.id).subscribe({
      next: (v) => this,
